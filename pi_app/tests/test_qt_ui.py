...
class QmlContractTests(unittest.TestCase):
    def test_all_seven_design_routes_are_native_qml_components(self):
        ...
        self.assertIn("function recoverCameras()", harness)
    ...
    def test_library_is_scrollable_and_primary_touch_targets_are_not_tiny(self):
        ...
        self.assertIn("property bool iconOnly: false", touch_button)
        self.assertIn("property real iconScale: 0.72", touch_button)
        self.assertIn("Math.min(control.width, control.height) * control.iconScale", touch_button)
        self.assertIn('objectName: "homeButton"', library)
        self.assertIn('label: "\\u2302"', library)
        self.assertIn("iconScale: 1.0", library)
        self.assertIn('onTapped: bridge.navigate("ready")', library)
        self.assertNotIn('label: "BACK TO CAMERA"', library)
