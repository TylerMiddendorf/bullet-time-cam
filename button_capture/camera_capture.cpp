...
bool capturePhoto() {
  ...
  if (frame->format != PIXFORMAT_JPEG) {
    sendNodeMessage(MSG_ERROR, sequence, esp_timer_get_time(),
                    "FRAME_NOT_JPEG");
  } else if (frame->width != CAPTURE_WIDTH ||
    ...
    snprintf(
        imageMetadata, sizeof(imageMetadata),
        "{\"node_uid\":\"%s\",\"boot_id\":%lu,\"capture_seq\":%lu,"
    ...
    const bool transferred =
        sendFrame(MSG_IMAGE, imageMetadata, frame->buf, frame->len,
                  consumeCorruptNextUsbImage());
    ...
    sendFrame(transferred ? MSG_TRANSFER_COMPLETE : MSG_ERROR,
              completeMetadata);
...
void initializeCamera() {
  ...
  if (sensor->set_quality) sensor->set_quality(sensor, JPEG_QUALITY);
  ...
  if (sensor->set_sharpness) sensor->set_sharpness(sensor, SENSOR_SHARPNESS);
  if (sensor->set_denoise) sensor->set_denoise(sensor, SENSOR_DENOISE);
