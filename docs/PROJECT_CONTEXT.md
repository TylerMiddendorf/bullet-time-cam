...
- A July 16 product-boot trial installed Raspberry Pi's early fullscreen logo support plus a custom Plymouth script theme. Plymouth 24.004.60 crashed in `libply-splash-core`/`libply` before the Pi reached networking or the camera application. The failure is unresolved on the bench Pi pending boot-card recovery; the repository follow-up removes the custom Plymouth layer and explicitly disables Plymouth while retaining the supported early logo and app-owned handoff. This boot path is not yet verified.
