...
- The product owner connected the trigger circuit while powered and explicitly skipped the required unpowered multimeter checks. Powered functional evidence is strong, but resistance, continuity, isolation, and transistor pinout were not independently verified. Electrical power remains unmeasured.
...
The functional physical-shutter and Pi GPIO17 trigger gates have passed separately on all four nodes, including repeated timing/integrity evidence. Checkpoint 4 remains open because the prescribed unpowered electrical inspection was skipped; it must be completed or explicitly waived. The next software step is product-level four-node grouping, atomic set persistence, and live multi-image GIF review. Battery integration and enclosure work follow only after the bench path and power/performance requirements are sufficiently measured. See `ROADMAP.md` and `MILESTONE_1_PLAN.md`.
...
- Camera-node local storage and GPIO-driven status indication are removed from firmware 0.2.0; all four nodes passed revised startup and physical/Pi-trigger functional capture gates on July 17. The skipped unpowered circuit inspection remains unresolved.
...
- Complete or explicitly waive the missing unpowered electrical inspection for the functionally validated Raspberry Pi BCM GPIO17/2N3904 trigger circuit.
