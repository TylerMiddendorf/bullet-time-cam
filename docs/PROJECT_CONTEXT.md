...
- July 16 product-boot trials found two boot blockers on the current Raspberry Pi OS Trixie/kernel build. A custom Plymouth script theme crashed Plymouth 24.004.60 in `libply-splash-core`/`libply`; after SD-card recovery, a second trial with Plymouth disabled but Raspberry Pi's initramfs early-fullscreen-logo path produced black/no-signal output and never reached networking. The repository follow-up disables both mechanisms, all HDMI console/status output, and desktop chrome; the stable target is a black pre-graphical display followed by the logo compositor/app first frame. This fallback still requires SD-card recovery and cold-boot verification and does not yet satisfy the stronger logo-throughout-boot requirement.
