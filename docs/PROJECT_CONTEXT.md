...
- July 16 product-boot trials found three constraints on the current Raspberry Pi OS Trixie/kernel build. A custom Plymouth script theme crashed Plymouth 24.004.60 in `libply-splash-core`/`libply`; Raspberry Pi's initramfs early-fullscreen-logo path produced black/no-signal output and never reached networking; after the generated hook/package were removed and initramfs rebuilt, a console-less boot still failed to reach networking while the otherwise identical `console=tty1` recovery boot succeeded. The repository follow-up disables both splash mechanisms and desktop chrome, retains one masked/silenced `tty1` console for boot compatibility, and targets a black pre-graphical display followed by the logo compositor/app first frame. This fallback still requires cold-boot verification and does not yet satisfy the stronger logo-throughout-boot requirement.
